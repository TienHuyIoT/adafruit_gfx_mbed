...
void SPIClass::beginTransaction(SPISettings settings)
{
  ...
  setClockDivider(64000000 / settings.clockFreq);
...
template<class T, class L> 
  auto _min(const T& a, const L& b) -> decltype((b < a) ? b : a)
  {
    return (b < a) ? b : a;
  }
...
void SPIClass::transfer(const void *tx_buf, void *rx_buf, size_t count)
{
  ...
  while (count)
  {
    ...
    const size_t xfer_len = _min(count, UINT16_MAX);
...
SPIClass SPIMode(_SPI_DEV,  PIN_SPI_MISO,  PIN_SPI_SCK,  PIN_SPI_MOSI);
#endif

#if SPI_INTERFACES_COUNT >= 2
SPIClass SPIMode1(_SPI1_DEV, PIN_SPI1_MISO, PIN_SPI1_SCK, PIN_SPI1_MOSI);
#endif
